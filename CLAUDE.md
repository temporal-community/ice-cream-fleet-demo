...
- **Server uses Temporal queries** (`server.py`): WebSocket data comes from querying
  `MeltdownDemoWorkflow.get_status` and `DriverRouteWorkflow.get_status` via
  `_build_snapshot_from_queries()`. FleetState is not imported by the server.
...
- **FleetState** (`simulation.py`): write-only UI projection used by mock activities only.
  Server does not read it — all frontend state comes from Temporal queries.
...
- **ADK agents** (`agents.py`): Fleet Agent + Customer Agent (parallel) → Resolver (sequential).
  ...
  if an activity fails, Temporal retries.
