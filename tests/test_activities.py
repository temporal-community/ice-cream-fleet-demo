...
async def test_navigate_to_interpolates_position(env: ActivityEnvironment):
    ...
    lat, lng = await fleet.get_driver_position("driver-1")
...
async def test_pickup_orders_sets_status(env: ActivityEnvironment):
    ...
    result = await env.run(
        pickup_orders,
        PickupInput(driver_id="driver-1", order_ids=["order-1"]),
    )
    assert result.success is True

    c = await fleet.get_driver("driver-1")
...
async def test_deliver_order_sets_status(env: ActivityEnvironment):
    ...
    result = await env.run(
        deliver_order,
        DeliverInput(driver_id="driver-1", order_id="order-1"),
    )
    assert result.success is True
